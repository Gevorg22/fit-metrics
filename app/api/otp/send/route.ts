...
export async function POST(req: Request) {
  try {
    ...
    const { error } = await resend.emails.send({
      ...
      html: `<div style="font-family:sans-serif;max-width:420px;margin:0 auto;padding:40px 24px;background:#171310;border-radius:16px;border:1px solid #3d2e24;"><h2 style="color:#ffa751;margin:0 0 20px;font-size:22px;">fitMetrics</h2><p style="color:#c2a184;margin:0 0 6px;font-size:14px;">Твой код для входа:</p><div style="font-size:42px;font-weight:700;letter-spacing:10px;color:#ffa751;margin:12px 0 20px;font-family:monospace;">${code}</div><p style="color:#8a6f57;font-size:12px;margin:0;">Код действителен 10 минут. Не передавай его никому.</p></div>`,
    });
